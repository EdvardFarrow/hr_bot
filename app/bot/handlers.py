...
async def handle_resume_pdf(message: Message, bot: Bot, state: FSMContext):
    ...
    if not text or len(text) < 50:
        await wait_msg.edit_text("Unable to read text from PDF. It may be a scan (image). Please send a text PDF or a link.")
        return
...
async def handle_resume_link(message: Message, state: FSMContext):
    ...
    if not text:
        await wait_msg.edit_text("I couldn't open the link (the profile might be private). It's better to send me the PDF file.")
        return
...
async def handle_any_text(message: Message, state: FSMContext):
    ...
    # If the state is unknown (for example, the user has not pressed start)   
        ...
        ai_answer = "Please type /start to begin the interview process."
